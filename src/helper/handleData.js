...
export const updateData = async (path, dispatch, payload) => {
    const response = await request("PUT", path, payload)
    dispatch({type: `UPDATE_${pathToType(path)}`, payload: response})
}

export const createData = async (path, dispatch, payload) => {
...
export const deleteData = async (path, dispatch, payload) => {
